...
import { onDelete } from '../../../actions/publicationActions';
import { clearNewPublication } from '../../../actions/publicationActions';
...
UNSAFE_componentWillReceiveProps(nextProps){
    if(nextProps.newPublication){
    
         if(nextProps.newPublication.id )//on verifie si y a un nouveau article pour afficher un alert
       {
        this.props.publications.unshift(nextProps.newPublication); //on ajoute le nouveau article en tete de liste
           console.log('New article added successfully !');
           console.log(nextProps.newPublication);
        this.setState({
            openAlert :true
        })
        this.props.clearNewPublication();
       } 
...
onSelectArticle = id =>{
    this.setState({
        localArticle : this.props.publications.find(article => article.id === id),
        editMode : false
    })
         ...
         return(
            ...
            <Grid container>
                <Grid item xm= {12} sm={12} md={4} >  <Paper style={styles.Paper} >
                        {this.props.publications.map(( blog, index) => 
                        <Fragment key={index}>
                            <Typography 
                                variant="h6"
...
Publications.propTypes = {
    ...
    clearNewPublication : PropTypes.func.isRequired,
...
export default connect(mapStateToProps,{ fetchPublications, clearNewPublication, onDelete })(Publications)
