...
export default function ListAnnonces(props) {
  const classes = useStyles2();
  const [page, setPage] = React.useState(0);
  const [rowsPerPage, setRowsPerPage] = React.useState(5);

  const store = useSelector(store => store);
  console.log(store.resultatsSearch);
  const resultatsSearch = store.resultatsSearch.items;
 
  const rows = [
   
    ].sort((a, b) => (a.calories < b.calories ? -1 : 1));
  resultatsSearch.map(( annonce, index) => rows.push(
      createData(
      <div key={index}>
